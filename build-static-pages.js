...
<h2 class="section-title">🏞️ 施設一覧</h2><ul class="facility-list">${list.map(li).join('')}</ul>
...
<h2 class="section-title">📍 区から探す</h2><div class="link-grid">${wards.map(w => card(`${BASE}${p.slug}/${w.slug}/`, w.name, w.count)).join('')}</div>
`;
  h += (direct.length ? `<h2 class="section-title">🏞️ 施設一覧</h2><ul class="facility-list">${direct.map(li).join('')}</ul>` : '');
  h += `\n<a class="cta" href="${BASE}">🧭 おでかけナビで検索・絞り込みして探す</a>\n</div>\n` + FOOTER;
  return h;
}
...
${blocks.map(b => `<div class="region-block" id="region-${b.p.slug}"><h2 class="region-title"><a href="${BASE}${b.p.slug}/">${esc(b.p.full)}</a>（${b.count}件）</h2><div class="region-sub">この地域のスポット一覧は<a href="${BASE}${b.p.slug}/">${esc(b.p.full)}のページ</a>でも見られます</div><ul class="facility-list">${b.list.map(li).join('')}</ul></div>`).join('')}
<a class="cta" href="${BASE}">🧭 おでかけナビで検索・絞り込みして探す</a>
</div>
` + FOOTER;
  return h;
...
function sitemapXml(prefs, rows) {
  // 日本語のURLはsitemapの仕様どおりパーセントエンコードして出力する（ページ側のcanonicalは従来のまま。同じURLとして扱われる）
  const enc = rel => encodeURI(rel).replace(/&/g, '&amp;');
  const ent = (rel, cf, pr) => `<url>\n<loc>${SITE}${enc(rel)}</loc>\n<changefreq>${cf}</changefreq>\n<priority>${pr}</priority>\n</url>\n`;
